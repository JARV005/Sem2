...
from rich import box  # ← Importante para los estilos de bordes
...
usuarios = { "1": {
    ...
    } }  # ← Aquí pones el diccionario completo que compartiste

# Crear consola
console = Console()

# Crear tabla
table = Table(title="Información de Usuarios (Organizado por Categorías)")

# Agregar columnas por categoría con colores
table.add_column("Nombre", style="green", no_wrap=True)
table.add_column("Usuario", style="green")
table.add_column("Nacimiento", style="green")
table.add_column("Ocupación", style="green")
...
table.add_column("Newsletter", style="yellow")
table.add_column("Intereses", style="yellow")

# Agregar filas
for x, data in usuarios.items():
    perfil = data["perfil"]
    ...
    table.add_row(
        ...
        preferencias["idioma"],
        "Sí" if preferencias["newsletter"] else "No",
        ", ".join(preferencias["temas_interes"])
    )
